...
class AuthService {
  ...
  async verifyOtp(userId, code) {
    ...
    const otp = otps[0];
    await db.query('UPDATE otp_codes SET consumed_at = NOW() WHERE otp_id = ?', [otp.otp_id]);
  ...
  async refresh(token) {
    ...
    if (tokens.length === 0) throw new UnauthorizedError('Invalid or expired refresh token');
